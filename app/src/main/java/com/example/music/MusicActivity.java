...
public class MusicActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public String[] Musicname={"陶喆——小镇姑娘","方大同——特别的人","周杰伦——听妈妈的话","王力宏——花田错","郭一凡——有我呢","李荣浩——不将就"};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        IntentFilter filter = new IntentFilter("com.example.music.UPDATE_UI");
        registerReceiver(new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                int position = intent.getIntExtra("position", 0);
                // Update the song name and image
                name_song.setText(Musicname[position]);
    ...
    public void onClick(View v) {
        if(v.getId()==R.id.btn_play)//播放按钮点击事件
        {
            String position=intent1.getStringExtra("position");
            int i=parseInt(position);
            musicControl.play(i);
            animator.start();
        } else if (v.getId()==R.id.btn_pause) {//暂停按钮点击事件
            musicControl.pausePlay();
            animator.pause();
        } else if (v.getId()==R.id.btn_continue_play) {//继续播放按钮点击事件
            musicControl.continuePlay();
            animator.start();
        } else if (v.getId()==R.id.btn_exit) {//退出按钮点击事件
            unbind(isUnbind);
            isUnbind=true;
            finish();
        } else if (v.getId()==R.id.btn_pre) {//上一首按钮点击事件
            musicControl.playPrevious();
        } else if (v.getId()==R.id.btn_next) {//下一首按钮点击事件
